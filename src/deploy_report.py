...
def main():
    ...
    root = Path(__file__).parent.parent / "report" / f"{report_name}.Report"

    print_step(1, 3, f"Build the PBIR folder for '{report_name}'")
    n_visuals = write_pbir(root, ws_name, model_name, model_id)
    print(f"   3 pages, {n_visuals} visuals -> {root.relative_to(root.parent.parent)}")
