...
from fabric._shared.paths import CONFIG_FILE, STATE_FILE
...
def load_config() -> Dict[str, Any]:
    if not CONFIG_FILE.exists():
        raise FileNotFoundError(
            f"{CONFIG_FILE} not found. Copy config.example.yaml to config.yaml "
...
def require_config(cfg: Dict[str, Any], key: str) -> Any:
    ...
    if _is_placeholder(value):
        ...
        raise RuntimeError(
            f"Missing '{key}': set it in config.yaml{hint}. "
...
def require_state(state: Dict[str, Any], key: str) -> Any:
    ...
    if _is_placeholder(value):
        ...
        hint = f" or export {env_var}" if env_var else ""
        raise RuntimeError(
            f"Missing '{key}' in state.json{hint}. "
