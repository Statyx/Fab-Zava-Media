...
— into every deploy script. ``winreg`` is a Windows-only stdlib module, so
...
Canonical prologue
------------------
Every deploy script that needs a working ``PATH`` starts with exactly these
...
That import is absolute, which is what pins the invocation model: deploy code is
grouped one package per workload (``fabric/lakehouse/``, ``foundry/``, …), so a
script only resolves its siblings when the repository root is on ``sys.path``. Run
them as modules from the root — ``python -m fabric.lakehouse.deploy_lakehouse`` — or
through ``python deploy_all.py``, which is at the root and therefore puts it there.
``python fabric/lakehouse/deploy_lakehouse.py`` does **not** work, by design: the
alternative was a ``sys.path`` fix-up in every file, which is exactly the kind of
copy-pasted platform glue this module exists to delete.
