...
import requests
from fabric._shared.helpers import (load_config, load_state, get_kusto_token, kusto_mgmt,
...
def ingest_csv(quri, ktok, db, table):
    ...
    if not path.exists():
        raise FileNotFoundError(f"{path} missing — run `python -m design.notebooks.generate_data` first.")
